...
void setup() {
  // Konfigurasi Pin yang ditentukan untuk berperilaku tertentu, baik yang sebagai INPUT_PULLUP ataupun OUTPUT
  pinMode(pinTombolMotor, INPUT_PULLUP);
  pinMode(pinTombolPemanas, INPUT_PULLUP);
  pinMode(pinMotor, OUTPUT);
  pinMode(pinPemanas, OUTPUT);

  dht.begin(); // Memulai pembacaan data oleh Sensor Temperatur DHT11
  ...
  lcd.print("Faumation"); // Menampilkan nama proyek saat dimuat pertama kali
  delay(3000);
}
...
void loop() {
  ...
  jarak = pulseIn(pinEcho, HIGH); // Menunggu hingga Pin Echo bernilai HIGH
...
long jarakCentimeter(long ns) {
  return ns / 29 / 2; // Kecepatan suara 29 microseconds per centimeter, lalu dibagi 2 untuk menemukan jarak objek dari jarak yang ditempuh
}